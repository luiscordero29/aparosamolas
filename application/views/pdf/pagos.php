...
$obj_pdf->Cell(0, 0, 'IMPAGOS', 0, 1, 'C', 0, '', 0,  0, '', 0);
$obj_pdf->Ln(5);
...
$obj_pdf->SetFont('helvetica', 'B', 12);
$obj_pdf->Cell(0, 0, 'ESTUDIANTE', 1, 1, 'C', 0, '', 0,  0, '', 0);
...
$obj_pdf->SetFont('helvetica', 'B', 12);
$obj_pdf->Cell(0, 0, 'IMPORTE INPAGADO', 1, 1, 'C', 0, '', 0,  0, '', 0);

if ($row['porcentajes'] == '0') 
{
    $monto = $row['precio'] - $row['precio']*$row['descuento']*0.01;
    if ($row['colegio'] == 'NO') {
        $monto = $monto + 50;
    }
}
if ($row['porcentajes'] == '50') {
    $monto = ($row['precio'] - $row['precio']*$row['descuento']*0.01)*0.5;
    if ($row['colegio'] == 'NO') {
        $monto = $monto + 25;
    }
}
if ($row['porcentajes'] == '100') {
    $monto = 0;
}

$obj_pdf->SetFont('helvetica', '', 10);
$obj_pdf->Cell(0, 0, number_format($monto, 2, ',', '.')." EUR", 1, 1, 'L', 0, '', 0,  0, '', 0);



// Close and output PDF document
// This method has several options, check the source code documentation for more information.*/
$obj_pdf->Output('LISTADOS-'.$row['id_inscripcion'].'.pdf', 'I');
